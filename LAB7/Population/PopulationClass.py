...
class Population:
    def __init__(self, populationSize):
        ...
        self.__individualsList = [Individual() for i in range(0, populationSize)]

    def evaluate(self, problem):
        evaluation = []
        for i in range(self.__populationSize):
            evaluation.append([i, self.__individualsList[i].computeFitness(problem.getInTrainData(), problem.getOutTrainData(), problem.getNoTrainExamples())])
        return evaluation
